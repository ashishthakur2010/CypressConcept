...
describe('Read and Write File Suite', () => {
	...
	it.only('Read a Test file at specific Path', () => {
		cy.readFile('cypress/fixtures/myresume.txt').should(
			'contain',
			'This is my resume',
		)
		cy.readFile('C:\\Users\\Ashish\\Documents\\resume.txt').should(
			'contain',
			'resume',
		)
	})

	it.only('loads the same object', () => {
		...
		cy.fixture('usercopy').then((userFixture) => {
			...
			cy.wrap(userFixture.user)
				.its('firstName')
				.should('contain', 'Ashish')
		})
	})
})
