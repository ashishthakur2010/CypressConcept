...
describe('DOM Commands', function () {
	it('Click on first() index', function () {
		cy.get('.products').children().debug().first().click()
	})
	...
	it('Find the closest() ancestor of class .product', function () {
		...
		cy.get('.product')
			.closest('.container')
			.should('have.class', 'container') //cy.get('selector').closest('.grandparent').should('have.class', 'grandparent');
	})
	...
	it('should have the correct title using document()', () => {
		cy.document()
			.its('title')
			.should('eq', 'GreenKart - veg and fruits kart')
	...
	it('Click using filter(":contains()")', function () {
		cy.get('.products .product h4')
			.filter(":contains('Banana - 1 Kg')")
			.click()
	})
				...
				if (item.innerText.includes('75') && price > 15) {
					cy.wrap(item)
						...
						.then(($prev) => {
							...
							expect(productName.split(' - ')[0]).to.equal(
								expectedText,
							)
						})
